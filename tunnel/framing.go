...
// # WHY THIS EXISTS
//
// The tunnel is a BYTE STREAM for TCP targets:
//
//	client ──DATA(msg)──► server ──write──► tcp target
//	client ◄──DATA(chunk)── server ◄──read── tcp target
//
// The client→target direction preserves boundaries (each DATA record becomes
// one TCP write), but the target→client direction does not: the server reads
// the target socket in budget-sized chunks and ships each chunk as its own DATA
// record, so a reply may arrive split or coalesced regardless of how it was
...
//
// (With a `udp://` target the tunnel DOES preserve datagram boundaries end to
// end: one datagram in, one DATA record out. Framing is only needed for stream
// targets, or when the client application wants message semantics.)
//
// The wire format is deliberately boring: a 4-byte big-endian length prefix
...
// MessageAssembler.
//
// The server's built-in `discard://` MTU sink reuses this exact framing, so the
// MTU probe adds no new wire format (see DESIGN_ICMP.md §4.8, option A′).
const (
...
// client batches requests into a single DATA record.
...
//	for {
//	    _, payload := client.readData()      // one DATA record, possibly partial
