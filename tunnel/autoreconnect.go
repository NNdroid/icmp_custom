...
func (pipeAddr) Network() string { return "icmp" }
...
func (c *Client) dialSession(ctx context.Context, opts DialOptions) (net.Conn, *clientSession, error) {
	...
	// The receive loop must be up before the SYN flies: the handshake ACK is
	// consumed by it (startRecvLoop is idempotent).
	c.startRecvLoop()
