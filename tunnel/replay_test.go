...
// Seq 0 is reserved for handshake records and never a valid session sequence.
...
// TestReplayFilterBigJumpClearsWindow covers the diff >= window fast path: a
// very large forward jump must forget the whole old bitmap rather than shifting
// one bit at a time.
func TestReplayFilterBigJumpClearsWindow(t *testing.T) {
	rf := &ReplayFilter{}
	if !rf.Accept(1) {
		t.Fatal("first packet must be accepted")
	}
	if !rf.Accept(1 + replayWindowBits + 100) {
		t.Fatal("large forward jump must be accepted")
	}
	if !rf.Seen(1) {
		t.Fatal("pre-jump packet must now be outside the window")
	}
	if rf.Accept(1 + replayWindowBits + 100) {
...
func TestReplayFilterRemoveOutOfRangeIsNoop(t *testing.T) {
	rf := &ReplayFilter{}
	rf.Remove(0) // reserved
	rf.Remove(99)
	if !rf.Accept(1) {
		t.Fatal("out-of-range Remove must not disturb the filter")
	}
	rf.Remove(1 + replayWindowBits)
	if !rf.Seen(1) {
		t.Fatal("Remove of an already-forgotten packet must be a no-op")
	}
}

func TestReplayFilterWindowBoundaryExact(t *testing.T) {
	rf := &ReplayFilter{}
	if !rf.Accept(1) {
		t.Fatal("seed accept failed")
	}
	// Exactly window-1 behind: still inside, so it must be rejected as too old
	// only once we move past the boundary.
	if !rf.Accept(1 + replayWindowBits - 1) {
		t.Fatal("packet at the inner edge must be accepted")
	}
	if !rf.Seen(1) {
		t.Fatal("seq 1 is exactly window-1 behind the new max and must be visible")
	}
}
